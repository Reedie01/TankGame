...
public class Shell : MonoBehaviour
{
    ...
    // Reference to the particls that will play on
    ...
    private void OnCollisionEnter(Collision other)
    {
        ...
        if (targetRigidbody != null)
        {
           ...
           transform.position, m_ExplosionRadius);
            ...
            EnemyHealth e_TargetHealth = targetRigidbody.GetComponent<EnemyHealth>();
            TankHealth p_TargetHealth = targetRigidbody.GetComponent<TankHealth>();
            if (p_TargetHealth != null)
            {
                // Calculate the amount of damage the target should take
                // based on it's distance from the shell.
                float damage = CalculateDamage(targetRigidbody.position);
                // Deal this damage to the tank
                p_TargetHealth.TakeDamage(damage);
            }
            else if(e_TargetHealth != null)
            {
                // Calculate the amount of damage the target should take
                // based on it's distance from the shell.
                float damage = CalculateDamage(targetRigidbody.position);
                // Deal this damage to the tank
                e_TargetHealth.TakeDamage(damage);
            }
        }
